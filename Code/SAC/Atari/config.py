...
from gymnasium.wrappers import TransformReward, AtariPreprocessing
...
class Config(Serialisable):
    ...
    # SAC-specific configuration fields
    algorithm: str = "sac"
    sac_lr: float = 0.0005
    ...
    sac_alpha: float = 0.2
    sac_alpha_lr: float = 0.0005
    sac_grad_clip: float = 5.0
    sac_target_entropy_coef: float = 0.8
    sac_alpha_min: float = 1e-06
    sac_alpha_max: float = 0.5
    sac_tes_enabled: bool = True
    sac_tes_lambda: float = 0.999
    sac_tes_avg_threshold: float = 0.01
    sac_tes_std_threshold: float = 0.05
    sac_tes_discount_k: float = 0.98
    sac_tes_T: int = 1000
    sac_tes_start_epoch: int = 10
    sac_collect_eps: float = 0.1
    sac_collect_early_eps_epochs: int = 60
    sac_policy_lr: float = 0.0005
    sac_eval_every: int = 10
    sac_eval_entropy_threshold: float = 2.3
    sac_eval_stochastic_temp: float = 0.5
    sac_warmup_min_sequences: int = 1024
    
    # Additional fields found in config files
    img_size: int = 64
