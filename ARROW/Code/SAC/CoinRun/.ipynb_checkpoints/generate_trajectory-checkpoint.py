...
def evaluate(
    ...
    # Deterministic (argmax) or stochastic (sample) step-wise evaluation.
    ...
    while len(ep_returns) < n_rollouts:
        if ac is not None:
            ...
            logits = ac(obs_t)
            # Deterministic policy-greedy with tiny tie-breaking noise to avoid constant-action traps
            noise = 1e-6 * torch.randn_like(logits)
            act = (logits + noise).argmax(dim=-1).cpu().numpy()
        else:
...
def generate_trajectories(
    ...
    if env_fns is None:
        print(f"[DEBUG] generate_trajectories: SyncVectorEnvAtHome")
    ...
    with tqdm(total=n, desc=f"Generating trajectories{post}", disable=True) as progbar:
        while n_samples < n:
            ...
            # 1) If an actor-critic with WM is provided, use it as before
            # 2) Else if a discrete actor (categorical policy) is provided, use it directly on images
            # 3) Else random actions
            if wm is not None and ac is not None:
                if z is None:
                ...
                with torch.no_grad():
                    ...
                    obs_t = torch.from_numpy(obs / 255).float().permute(0, 3, 1, 2).to(policy_device)
                    logits = ac(obs_t)
                    # Sampled debug: first few steps and then very sparse cadence; only first few envs
                    if deterministic:
