...
# Ensure third-party envs that still register with 'gym' work under Gymnasium
...
class EnvConfig(Serialisable):
    ...
    def get_function(self) -> Callable[[], Any]:
        ...
        if parts[0] == "Coinrun": # some files were writting in lowercase
            parts[0] == "CoinRun"
...
class Config(Serialisable):
    ...
    sac_dv3_data_n_max: int = 1024 # to match total memory for ARROW
    sac_tau: float = 0.005
    sac_gamma: float = 0.99
    sac_alpha: float = 0.2
    sac_alpha_lr: float = 1e-3
    sac_grad_clip: float = 20.0
    # Target entropy for discrete SAC: -coef * log(|A|)
    sac_target_entropy_coef: float = 1.0
    # Bounds for entropy coefficient when auto-tuning (exp(log_alpha))
    sac_alpha_min: float = 1e-6
    sac_alpha_max: float = 1.0
    # Target Entropy Scheduling (TES-SAC)
    ...
    # Data collection exploration (epsilon-greedy) for early epochs
    sac_collect_eps: float = 0.05
    sac_collect_early_eps_epochs: int = 15
    # Optional faster actor learning rate; defaults to sac_lr when not provided
    sac_policy_lr: float = 3e-4
    # Evaluation controls
    sac_eval_every: int = 1
    sac_eval_entropy_threshold: float = 2.3
    sac_eval_stochastic_temp: float = 0.5
    # Warmup: require at least this many sequences in replay before doing updates
    # Note: replay capacity is measured in sequences; each sequence has length `data_t`
