...
def train_sac(config: Config):
    ...
    print(f"[DEBUG] train_sac: Creating encoders and heads (shared for Q1/Q2; separate for policy)...")
    ...
    print(f"[DEBUG] train_sac: Optimizers initialized with lr={config.sac_lr} (policy_lr={policy_lr})")
    ...
    # Exploration recovery state
    rec_enabled = getattr(config, "sac_explore_recovery_enabled", True)
    rec_patience = getattr(config, "sac_explore_recovery_patience_epochs", 3)
    rec_min_eval = getattr(config, "sac_explore_recovery_min_eval_mean", 0.1)
    rec_min_train = getattr(config, "sac_explore_recovery_min_train_rews", 0.2)
    rec_alpha = getattr(config, "sac_explore_recovery_alpha", 0.02)
    rec_duration = getattr(config, "sac_explore_recovery_duration_epochs", 3)
    rec_cooldown = getattr(config, "sac_explore_recovery_cooldown_epochs", 10)
    rec_te_coef = getattr(config, "sac_explore_recovery_target_entropy_coef", 1.2)
    rec_active = False
    rec_active_until = -1
    rec_cooldown_until = -1
    rec_train_low_streak = 0
    rec_eval_low_streak = 0
    rec_prev_target_entropy = float(target_entropy)
    base_target_entropy = float(target_entropy)
        ...
        while replay_buffer.n_valid < config.sac_warmup_min_sequences:
            acts, obss, rews, conts, resets = reinterpret_nt_to_t_n(
                *generate_trajectories(
                    ...
                    collect_eps=config.sac_collect_eps,
                ),
                config.data_t,
                config.data_n,
            )
    ...
    for epoch in range(config.epochs):
        ...
        # Exploration recovery detection (based on train rewards)
        if rec_enabled and epoch >= rec_cooldown_until:
            if rews_eps_mean <= rec_min_train and float(log_alpha.exp().item()) <= getattr(config, "sac_alpha_min", 1e-6) * 1.01:
                rec_train_low_streak += 1
            else:
                rec_train_low_streak = 0
        ...
        if epoch % 10 == 0:
            ...
            for i, efns in enumerate(eval_funcs):
                ...
                print(f"[DEBUG] train_sac: Env {i+1} eval - (mean={m_pol:.3f}, std={s_pol:.3f})")
            ...
            # Exploration recovery detection (based on eval rewards)
            if rec_enabled and epoch >= rec_cooldown_until:
                if approx_perf <= rec_min_eval and float(log_alpha.exp().item()) <= getattr(config, "sac_alpha_min", 1e-6) * 1.01:
                    rec_eval_low_streak += 1
                else:
                    rec_eval_low_streak = 0
        
        schedule.step()

        # Trigger exploration recovery if needed
        if rec_enabled and not rec_active and epoch >= rec_cooldown_until:
            if rec_train_low_streak >= rec_patience or rec_eval_low_streak >= rec_patience:
                rec_active = True
                rec_active_until = epoch + rec_duration
                rec_cooldown_until = rec_active_until + rec_cooldown
                rec_prev_target_entropy = float(target_entropy)
                # Temporarily increase alpha and target entropy
                with torch.no_grad():
                    log_alpha[...] = torch.log(torch.tensor(rec_alpha, device=log_alpha.device))
                target_entropy = max(float(base_target_entropy), float(rec_prev_target_entropy) * rec_te_coef)
                print(f"[DEBUG] RECOVERY: activated for epochs [{epoch}, {rec_active_until}) | alpha->{rec_alpha:.6f}, target_entropy {rec_prev_target_entropy:.4f}->{target_entropy:.4f}")

        # Deactivate exploration recovery after duration
        if rec_enabled and rec_active and epoch >= rec_active_until:
            rec_active = False
            # Restore target entropy towards pre-recovery value
            target_entropy = float(rec_prev_target_entropy)
            print(f"[DEBUG] RECOVERY: deactivated at epoch {epoch} | target_entropy restored to {target_entropy:.4f}; cooldown until {rec_cooldown_until}")
        ...
        for step in range(config.steps_per_batch):
            ...
            # Pause TES adjustments during active recovery; still update alpha towards current target_entropy
            ...
            with torch.no_grad():
                ...
                if tes_enabled and (epoch >= tes_start_epoch) and not rec_active:
