...
def train_sac(config: Config):
    print(f"[DEBUG] train_sac: Starting SAC training with seed={config.seed}")
    torch.manual_seed(config.seed)
    np.random.seed(config.seed)
    print(f"[DEBUG] train_sac: Seeds set successfully")

    # build run_name & log_dir
    current_time = datetime.now().strftime("%b%d_%H-%M-%S")
    run_name = f"{current_time}_{socket.gethostname()}"
    print(f"[DEBUG] train_sac: run_name={run_name}")
    # Use absolute log directory to avoid issues on HPC with changing CWD or cleanup
    log_root = Path.cwd() / "runs" / "sac"
    ...
    print(f"[DEBUG] train_sac: TensorBoard writer initialized")

    # Device selection (prefer CUDA if available)
    cuda_available = torch.cuda.is_available()
    if cuda_available:
        print(f"[DEBUG] train_sac: CUDA is available, device count: {torch.cuda.device_count()}")
        try:
            print(f"[DEBUG] train_sac: Current CUDA device: {torch.cuda.current_device()}")
            print(f"[DEBUG] train_sac: CUDA initialization successful")
        except Exception as e:
            print(f"[DEBUG] train_sac: CUDA query failed: {e}")
    device = torch.device("cuda" if cuda_available else "cpu")
    print(f"[DEBUG] train_sac: Using device: {device.type}")
    ...
    if device.type == "cuda":
        ...
        alloc_gb = torch.cuda.memory_allocated(dev_index) / 1e9
        reserved_gb = torch.cuda.memory_reserved(dev_index) / 1e9
        peak_alloc_gb = torch.cuda.max_memory_allocated(dev_index) / 1e9
        used_gb = used_bytes / 1e9
        total_gb = total_bytes / 1e9
        
        print(f"[DEBUG] CUDA mem: used={used_gb:.2f}GB/{total_gb:.2f}GB | "
            f"allocated={alloc_gb:.2f}GB reserved={reserved_gb:.2f}GB peak_alloc={peak_alloc_gb:.2f}GB")

    schedule = config.get_env_schedule()
    replay_buffer = config.get_replay_buffer()
    print(f"[DEBUG] train_sac: replay buffer device: {replay_buffer.obss.device}")
    ...
    print(f"[DEBUG] train_sac: Input channels={in_ch}, Action dim={act_dim}")
    
    print(f"[DEBUG] train_sac: Creating encoders and heads (shared Q encoder, separate actor encoder)...")
    q_shared_encoder = Encoder(img_channels=in_ch, channels=config.cnn_depth).to(device)
    # Actor gets its own encoder
    policy = CategoricalPolicy(in_ch, act_dim, config, encoder=None).to(device)
    # Critics share a single encoder
    q1 = QNetwork(in_ch, act_dim, config, encoder=q_shared_encoder).to(device)
    q2 = QNetwork(in_ch, act_dim, config, encoder=q_shared_encoder).to(device)
    # Target critics keep their own separate encoders
    target_q1 = QNetwork(in_ch, act_dim, config, encoder=None).to(device)
    target_q2 = QNetwork(in_ch, act_dim, config, encoder=None).to(device)
    print(f"[DEBUG] train_sac: All networks created and moved to {device.type}")
    
    target_q1.load_state_dict(q1.state_dict())
    target_q2.load_state_dict(q2.state_dict())
    print(f"[DEBUG] train_sac: Target networks initialized with main network weights")

    # Same LR for actor and critics; optional cosine decay schedulers
    ...
    policy_sched = CosineAnnealingLR(policy_opt, T_max=config.epochs, eta_min=max(config.sac_lr * 0.2, 1e-5))
    q1_sched = CosineAnnealingLR(q1_opt, T_max=config.epochs, eta_min=max(config.sac_lr * 0.2, 1e-5))
    q2_sched = CosineAnnealingLR(q2_opt, T_max=config.epochs, eta_min=max(config.sac_lr * 0.2, 1e-5))
    ...
    print(f"[DEBUG] train_sac: Optimizers initialized with lr={config.sac_lr}")
    print(f"[DEBUG] train_sac: Policy optimizer param groups: {len(policy_opt.param_groups)}")
    print(f"[DEBUG] train_sac: Q1 optimizer param groups: {len(q1_opt.param_groups)}")
    print(f"[DEBUG] train_sac: Q2 optimizer param groups: {len(q2_opt.param_groups)}")
    ...
    # Pre-collection warmup outside epoch loop
    if replay_buffer.n_valid < config.sac_warmup_min_sequences:
        print(
            f"[DEBUG] train_sac: Warmup starting - sequences in replay={replay_buffer.n_valid}, "
            f"target={config.sac_warmup_min_sequences}"
        )
        while replay_buffer.n_valid < config.sac_warmup_min_sequences:
            ...
            print(
                f"[DEBUG] train_sac: Warmup progress - sequences={replay_buffer.n_valid}/"
                f"{config.sac_warmup_min_sequences}, total_env_steps={total_env_steps}"
            )
        print(f"[DEBUG] train_sac: Warmup complete - sequences in replay={replay_buffer.n_valid}")

    for epoch in range(config.epochs):
        print(f"\n[DEBUG] ===== EPOCH {epoch}/{config.epochs} STARTING =====\n")

        # DATA COLLECTION
        print(f"[DEBUG] train_sac: Starting data collection for epoch {epoch}")
        print(f"[DEBUG] train_sac: Collection params - n_sync={config.n_sync}, gen_seq_len={config.gen_seq_len}, env_repeat={config.env_repeat}")
        print(f"[DEBUG] train_sac: Total trajectory length: {config.n_sync * config.gen_seq_len}")
        ...
        print(f"[DEBUG] train_sac: Generated trajectories - acts.shape={acts.shape}, obss.shape={obss.shape}")
        ...
        if epoch % 1 == 0:
            ...
            for i, efns in enumerate(eval_funcs):
                ...
                print(f"[DEBUG] train_sac: Environment {i+1} results - mean={m:.3f}, std={s:.3f}")
                eval_means.append(m)
                eval_stds.append(s)
        ...
        for step in range(config.steps_per_batch):
            ...
            # Debug removed inside update loop for performance/noise
        ...
        print(
            ...
            f"Q1_last={last_q1_loss:.6f}, Q2_last={last_q2_loss:.6f}, Policy_last={last_policy_loss:.6f}, "
            f"alpha={alpha_current:.6f}, policy_entropy_mean={entropy_mean:.6f}, policy_entropy_last={(last_entropy if last_entropy is not None else float('nan')):.6f}"
        )
        ...
        # Print CUDA memory usage (per-epoch)
        if device.type == "cuda":
            torch.cuda.synchronize()
            dev_index = torch.cuda.current_device()
            free_bytes, total_bytes = torch.cuda.mem_get_info(dev_index)
