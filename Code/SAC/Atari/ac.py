...
from rssm import ActionT, HiddenT, ImageT, LatentShape, LatentT, get_mlp_layers
from wm import ActionT, RewardSymlogT, RewardT, WorldModel, symexp, symlog

ActionLogT = torch.Tensor
AcState = torch.Tensor
...
def zh_to_ac_state(z: LatentT, h: HiddenT) -> AcState:
    return torch.cat((z.flatten(-2), h), dim=-1)


def ac_state_to_zh(state: AcState, ls: LatentShape, h_dim: int) -> Tuple[LatentT, HiddenT]:
...
class ActorCritic(nn.Module):
    def __init__(self, in_dim: int, act_space: int) -> None:
        super().__init__()
        self.actor: Callable[[AcState], ActionT] = nn.Sequential(
        ...
        self.critic: Callable[[AcState], RewardSymlogCatT] = nn.Sequential(
            *critic_fcs,
            nn.LogSoftmax(-1),
        )

    def __call__(self, state: AcState) -> Tuple[ActionT, RewardT]:
        return super().__call__(state)

    def forward(self, state: AcState) -> Tuple[ActionT, RewardT]:
...
def dream_rollout(
    ...
    ctx_acts, ctx_images, _, _, ctx_resets = data.minibatch(n_ctx_frames, n_sync, mb_device=z.device)
    assert ctx_images.shape == (n_ctx_frames, n_sync, 3, 64, 64), ctx_images.shape
    # for ctx_act, ctx_image, ctx_reset in zip(ctx_acts, ctx_images, ctx_resets):
    #     _, z, h = wm.rssm(z, ctx_act, h, ctx_image, ctx_reset, temperature=temperature)
...
if __name__ == "__main__":
    rews = torch.tensor([-19.9, -19.1, 18, 19.5]).unsqueeze(-1).float()
    res = rew_symlog_to_2hot(rews)
    print(res @ torch.linspace(-20, 20, N_CRITIC_BINS).float().unsqueeze(1))

    rews = (torch.rand(4, 3, 1) - 0.5) * 2 * 20
    res = rew_symlog_to_2hot(rews)
    print(rews)
    inv = res @ torch.linspace(-20, 20, N_CRITIC_BINS).float().unsqueeze(1)
    print(inv)
    assert rews.shape == inv.shape

    print((rews - inv).abs().mean() < 1e-6)
