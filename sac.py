...
def train_sac(config: Config):
    ...
    for epoch in range(config.epochs):
        ...
        for _ in range(config.ac_train_steps):
            # only train once we have enough in the buffer
            ...
            # Every log_frequency steps, dump all the “Metric/…” scalars
            if global_step % config.log_frequency == 0:
                writer.add_scalar("Metric/grad_norm",   grad_norm,         global_step)
                writer.add_scalar("Metric/Q1_loss",     q1_loss.item(),    global_step)
                writer.add_scalar("Metric/Q2_loss",     q2_loss.item(),    global_step)
                writer.add_scalar("Metric/Policy_loss", policy_loss.item(), global_step)
