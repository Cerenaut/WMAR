...
def train_sac(config: Config):
    ...
    log_root = Path.cwd() / "runs" / "reversed"
    ...
    # Fix: Use config.img_size instead of hardcoded 32x32
    dummy = torch.zeros(1, 3, config.img_size, config.img_size, device=device)
    in_ch = dummy.shape[1]
    act_dim = config.action_space
    
    print(f"SAC: Image dimensions: {config.img_size}x{config.img_size}, Channels: {in_ch}, Actions: {act_dim}")
    
    # Validate environment compatibility
    if config.esc.env_configs:
        env_name = config.esc.env_configs[0].name
        if "ALE" in env_name:
            print(f"SAC: Atari environment detected: {env_name}")
            if config.img_size != 64:
                print(f"[WARNING] Atari typically uses img_size=64, got {config.img_size}")
        else:
            print(f"SAC: Non-Atari environment: {env_name}")
    
    # Ensure all networks use consistent dimensions
    ...
    test_input = torch.zeros(1, in_ch, config.img_size, config.img_size, device=device)
    encoder_output = critic_shared_encoder(test_input)
    print(f"SAC: Encoder output size: {encoder_output.shape}")
    print(f"SAC: Expected MLP input size: {config.mlp_features}")
    
    # Validate network architectures
    print(f"SAC: Policy network - input: {encoder_output.shape[1]}, output: {act_dim}")
    print(f"SAC: Q networks - input: {encoder_output.shape[1]}, output: {act_dim}")
    ...
    for epoch in range(config.epochs):
        ...
        print(f"SAC: Generating trajectories - n_sync={config.n_sync}, gen_seq_len={config.gen_seq_len}")
        ...
        # Validate trajectory data shapes
        print(f"SAC: Trajectory shapes - acts: {acts.shape}, obss: {obss.shape}, rews: {rews.shape}")
        print(f"SAC: Expected shapes - acts: ({config.data_t}, {config.data_n}, {act_dim}), obss: ({config.data_t}, {config.data_n}, 3, {config.img_size}, {config.img_size})")
        ...
        if epoch % 10 == 0:
            ...
            print(f"train_sac: Eval mode = policy_greedy (deterministic)")
            ...
            for i, efns in enumerate(eval_funcs):
                ...
                print(f"train_sac: Env {i+1} eval - policy_greedy(mean={m_pol:.3f}, std={s_pol:.3f})")
        ...
        for step in range(config.steps_per_batch):
            ...
            # Validate minibatch data
            if step == 0:  # Only validate first step to avoid spam
                print(f"SAC: Minibatch shapes - acts: {mb_acts.shape}, obs: {mb_obs.shape}, rews: {mb_rews.shape}")
                print(f"SAC: Expected minibatch shapes - acts: (2, {config.sac_batch_size}, {act_dim}), obs: (2, {config.sac_batch_size}, 3, {config.img_size}, {config.img_size})")
            ...
            if step == 0:  # Only validate first step
                print(f"SAC: Observation shapes - s: {s.shape}, s_next: {s_next.shape}")
                print(f"SAC: Expected observation shape: ({config.sac_batch_size}, 3, {config.img_size}, {config.img_size})")
                if s.shape[-2:] != (config.img_size, config.img_size):
                    print(f"[ERROR] SAC: Observation dimensions mismatch! Expected {config.img_size}x{config.img_size}, got {s.shape[-2:]}")
                    raise ValueError(f"Observation dimensions mismatch: {s.shape} vs expected {config.img_size}x{config.img_size}")
            ...
            # Correct sign for gradient descent: minimize alpha * (target_entropy - entropy)
            # Detach the entropy target term so gradients only flow through log_alpha
        ...
        # Advance schedule once per epoch, after eval and updates
        schedule.step()
