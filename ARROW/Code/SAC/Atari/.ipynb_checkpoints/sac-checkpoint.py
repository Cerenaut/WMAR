...
def train_sac(config: Config):
    ...
    print(f"[DEBUG] SAC: Image dimensions: {config.img_size}x{config.img_size}, Channels: {in_ch}, Actions: {act_dim}")
    
    # Validate environment compatibility
    if config.esc.env_configs:
        env_name = config.esc.env_configs[0].name
        if "ALE" in env_name:
            print(f"[DEBUG] SAC: Atari environment detected: {env_name}")
            if config.img_size != 64:
                print(f"[WARNING] Atari typically uses img_size=64, got {config.img_size}")
        else:
            print(f"[DEBUG] SAC: Non-Atari environment: {env_name}")
    ...
    print(f"[DEBUG] SAC: Encoder output size: {encoder_output.shape}")
    print(f"[DEBUG] SAC: Expected MLP input size: {config.mlp_features}")
    
    # Validate network architectures
    print(f"[DEBUG] SAC: Policy network - input: {encoder_output.shape[1]}, output: {act_dim}")
    print(f"[DEBUG] SAC: Q networks - input: {encoder_output.shape[1]}, output: {act_dim}")
    ...
    for epoch in range(config.epochs):
        ...
        # Generate trajectories with proper debugging
        print(f"[DEBUG] SAC: Generating trajectories - n_sync={config.n_sync}, gen_seq_len={config.gen_seq_len}")
        acts, obss, rews, conts, resets = reinterpret_nt_to_t_n(
            *generate_trajectories(
                config.n_sync * config.gen_seq_len,
                config.n_sync,
        ...
        print(f"[DEBUG] SAC: Trajectory shapes - acts: {acts.shape}, obss: {obss.shape}, rews: {rews.shape}")
        print(f"[DEBUG] SAC: Expected shapes - acts: ({config.data_t}, {config.data_n}, {act_dim}), obss: ({config.data_t}, {config.data_n}, 3, {config.img_size}, {config.img_size})")
        ...
        for step in range(config.steps_per_batch):
            ...
            if step == 0:  # Only validate first step to avoid spam
                print(f"[DEBUG] SAC: Minibatch shapes - acts: {mb_acts.shape}, obs: {mb_obs.shape}, rews: {mb_rews.shape}")
                print(f"[DEBUG] SAC: Expected minibatch shapes - acts: (2, {config.sac_batch_size}, {act_dim}), obs: (2, {config.sac_batch_size}, 3, {config.img_size}, {config.img_size})")
            ...
            if step == 0:  # Only validate first step
                print(f"[DEBUG] SAC: Observation shapes - s: {s.shape}, s_next: {s_next.shape}")
                print(f"[DEBUG] SAC: Expected observation shape: ({config.sac_batch_size}, 3, {config.img_size}, {config.img_size})")
