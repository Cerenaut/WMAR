...
def evaluate(
    ...
    # DV3/ARROW-style: collect a big batch and compute returns from resets/conts
    # Use no_images=True for speed; rely on env_fns (already wrapped) for preprocessing
    ...
    if not eps_rews:
        ...
        return rews.sum().item() / n_eps, float(np.sqrt(var))
    return float(np.mean(eps_rews)), float(np.std(eps_rews))
...
def generate_trajectories(
    ...
    # Can't do it all in 1 array because sync env resets ruin everything!!!!!
    # We want to use a multi sync env because batching is good
    ...
    try:
        action_space_size = env.envs[0].action_space.n
    except Exception:
    ...
    with tqdm(total=n, desc=f"Generating trajectories{post}", disable=True) as progbar:
        while n_samples < n:
            ...
            n_samples += n_sync
            if wm is None and ac is not None:
                # Policy-only branch: use SAC policy directly on images
                ...
                if collect_eps > 0.0:
                    ...
                    if mask.any():
                        rand_act = np.random.randint(0, action_space_size, size=n_sync)
                        act = np.where(mask, rand_act, act)
            elif wm is None and ac is None:
                act = np.random.randint(0, action_space_size, size=n_sync)
            else:
                if z is None:
                    z, h = wm.rssm.initial_state(n_sync)
                    act_t = torch.zeros(n_sync, action_space_size, device=z.device)
                ...
                if deterministic:
                    act = torch.argmax(act_prob, dim=-1)
                else:
                    act_prob_dist = td.Categorical(logits=act_prob)
                    act = act_prob_dist.sample()
                act_t = torch.nn.functional.one_hot(act, action_space_size)
                act = act.cpu().numpy()
                # Epsilon-greedy exploration
                if collect_eps > 0.0:
                    explore_mask = np.random.rand(n_sync) < collect_eps
                    if explore_mask.any():
                        rand_act = np.random.randint(0, action_space_size, size=n_sync)
                        act = np.where(explore_mask, rand_act, act)
                        act_t = torch.nn.functional.one_hot(torch.from_numpy(act), action_space_size).to(act_t.device)
    ...
    return (
        torch.nn.functional.one_hot(torch.from_numpy(np.concatenate(acts)[:n]).long(), action_space_size).float(),
...
def reinterpret_nt_to_t_n(
    ...
    # obss may be None when no_images=True
    if obss is not None:
        obs_rest = tuple(obss.shape[1:])
        obss_tn = obss.reshape(n, t, *obs_rest).swapaxes(0, 1)
    else:
        obss_tn = None
    return (
        acts.reshape(n, t, action_dim).swapaxes(0, 1),
        obss_tn,
...
if __name__ == "__main__":
    evaluate(4, n_rollouts=100)

    acts, obss, rews, conts, resets = generate_trajectories(1024 * 4, 1, env_repeat=4)
    acts = acts.numpy().argmax(-1)
    obss = np.moveaxis(obss.numpy(), 1, -1)
    rews = rews.numpy()[..., 0]
    conts = conts.numpy()[..., 0]
    resets = resets.numpy()[..., 0]
    ...
    # Find a reset
    print(np.where(rews))
    w = np.where(rews == rews.max())[0][-1] + 1
    # w = np.where(resets)[0][-1] + 1
    print(w)
    print("rst", resets[w - 2 : w + 2])
    print("cnt", conts[w - 2 : w + 2])
    ...
    n, t = 1024, 4
    acts = acts.reshape(n, t).swapaxes(0, 1)
    obss = obss.reshape(n, t, 64, 64, 3).swapaxes(0, 1)
    rews = rews.reshape(n, t).swapaxes(0, 1)
    conts = conts.reshape(n, t).swapaxes(0, 1)
    resets = resets.reshape(n, t).swapaxes(0, 1)
    ...
    print(f"{rews.max()=}")
    print(f"{rews.mean()=}")
    print(f"{np.abs(rews).sum()=}")
    print(f"{resets.sum()=}")

    if wt > 1:
        plt.imshow(obss[wt - 1, wn])
        plt.show()
    else:
        plt.imshow(obss[3, wn - 1])
        plt.show()

    plt.imshow(obss[wt, wn])
    plt.show()
